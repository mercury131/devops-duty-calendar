...
def test_edit_task():
    ...
    filtred_html = (parsed_html.body.find('li', attrs={'class':'task', 'data-day': day0}))
    data_id=filtred_html['data-id']



    # Making a post request to edit new duty task
    files = {
    'project': (None, 'test'),
    'duty1': (None, 'duty02'),
    'duty2': (None, 'duty01'),
    ...
    response = s.post((url + '/' + calendar + '/' + year +  '/' + month + '/' + day0 + '/task/' + data_id),files=files)
    ...
    parsed_html = BeautifulSoup(response.content, "lxml")
    filtred_html = (parsed_html.body.find('li', attrs={'class':'task', 'data-day': next_day0}))
    data_id=filtred_html['data-id']    

    # Making a post request to edit second duty task
    files = {
    'project': (None, 'test'),
    'duty1': (None, 'duty02'),
    'duty2': (None, 'duty01'),
    ...
    response = s.post((url + '/' + calendar + '/' + year +  '/' + month + '/' + next_day0 + '/task/' + data_id),files=files)
    ...
    expected_result=('<li class="task" data-day="' + day0 + '" data-id="12345" data-month="' + month + '" data-year="' + year + '" style="background-color:#3EB34F"> test duty02 12345678901 <p class="accordion-hidden"> duty01 12345678901 <a class="button smaller remove-task"' )  
